...
class Profesor extends Conexion{
    ...
    public function ObtenerProfesorID(){
        if(isset($_POST['id_profesor'])){
            ...
            $pdo = $this->conectar();
            $query = $pdo->query("SELECT id, nombre, titulo, correo, id_materia FROM coaches WHERE id = $this->id");
            $query->execute();
            $result = $query->fetchAll(PDO::FETCH_ASSOC);
            return $result;
        }
    ...
    public function ActualizarEstado(){
        if(isset($_POST['id_profesor'], $_POST['estado'])){
            ...
            $result = $query->execute([$estado,$this->id]);
    ...
    public function PerfilProfesores(){
        $id = $_SESSION['id_profesor'];
        $pdo = $this->conectar();

        $query = $pdo->query("SELECT coaches.nombre, coaches.titulo, coaches.direccion, coaches.correo, materia.materia FROM coaches INNER JOIN materia ON coaches.id_materia = materia.id WHERE coaches.id = $id");
        $query->execute();
